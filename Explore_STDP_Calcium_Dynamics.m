...
paras = Param_InitVal_newCaV(paras);
paras.display = 0;
% paras.ratio_Cm = 10; % 10; % 10; % 10;
% paras.ratio_AMPAR = 1; % 0.0;
% paras.ratio_NMDAR = 1.048; % 1.0451; % 1.039;
...
hold on;
% dt_lst = [3000];
dt_lst = [-100, -20, 40, 100];
for k = 1:length(dt_lst)
    dt = dt_lst(k);
    [t_Ca_lst, Ca_lst, Ca_sys_lst] = run_simulation_k(tk_pre_lst, t_end, dt_lst(k), k, paras);
%     save(sprintf('Result/Result_04_28/BTSP/BTSP_dynamic_dt=%dms.mat', dt), 'dt', ...
%      't_Ca_lst', 'Ca_lst', 't_lst', 'y_lst', 'c_MP_lst', 'c_E_lst');
%     fig = figure(k);
    plot(t_Ca_lst, Ca_lst, 'DisplayName', sprintf('%d ms', dt), 'LineWidth',2.5,...
    'Color', color_lst(k,:));
...
set(axes1,'FontSize',28,'LineWidth',2.5, 'XTick',...
    [9.8, 9.9, 10, 10.1, 10.2 10.3],'XTickLabel',...
    {'-0.2','-0.1','0','0.1','0.2','0.3'},'XTickLabelRotation',45, 'YTick',...
    [0 0.2 0.4 0.6 0.8],'YTickLabel',{'0','0.2','0.4','0.6', '0.8'});
% 创建 legend
...
function [t_Ca_lst, Ca_lst, Ca_sys_lst] = run_simulation_k(tk_pre_lst, t_end, dt, k, paras)
...
circuit_model = @(t, y) V_dynamic_Brunel_stoc_newCaV(y, x_AMPA_func(t),...
                           x_NMDA_func(t), I_stim_func(t), ... % 0.418, 2);
                           0.412 , 2.02);

opts = odeset('RelTol',1e-7, 'AbsTol',1e-6, 'MaxStep',1e-3);
% dc_model = @(t, y) V_dynamic_Brunel_stoc(y, 0, 0, I_stim_func(t), 0.418, 2);
