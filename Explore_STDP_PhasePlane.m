...
c_E0_lst = zeros(N_trail, 1);
c_MP_lst = zeros(N_trail, 1);
AMPA_lst = zeros(N_trail, 1);

parfor k = 1:length(dt_lst)
    [c_E0, c_MP] = run_simulation_k(tk_pre_lst, t_end, dt_lst(k), k, paras);
    c_E0_lst(k) = c_E0;
    c_MP_lst(k) = c_MP;
%     E_lst(k) = E_SS;
%     AMPA_lst(k) = AMPA_SS;
...
% save("STDP_PhasePlane_Points_larger_range.mat", 'dt_lst', 'c_E0_lst', 'c_MP_lst');

%% Plot Results
figure(); scatter(c_MP_lst, c_E0_lst, 20, dt_lst, 'filled');
set(gca, 'xscale', 'log');
set(gca, 'yscale', 'log');
xlim([1e-6, 10]);
ylim([1e-2, 1e2]);
...
function [c_E0, c_MP] = run_simulation_k(tk_pre_lst, t_end, dt, k, paras)
sigma_NMDA = 3.3/100;
...
t = sol_Ca.x; y = sol_Ca.y'; y_Ca = y(:, 16);
cal_source = @(t) Ca_func(sol_Ca, t);

%% Initial state of CaMKII system

% Baseline SS
y0 = paras.y0_CaMKII; paras.c0 = paras.c0;
...
% if dt < 0
%     t_max = max([tk_post_lst, tk_pre_lst]) + 0.2;
% else
%     t_max = max([tk_post_lst, tk_pre_lst]) + 0.5;
% end

ind_t = find(y_Ca > 9.99e-2, 1, 'last');
t_max = t(ind_t);

ind_d_max = find(t_lst > t_max, 1);
y_ROI = y_lst(ind_d_max,:); 
c_Ca =  cal_source(t_max);
c_E0 = y_ROI(1); c_EP0 = y_ROI(3);
y_CaMKII_SS = y_ROI(5:19);
