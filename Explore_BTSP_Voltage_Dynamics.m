...
t_end = 40;
tk_pre_center = [15.45];
tk_pre_lst = [15:0.1:15.9]; % Pre-spike time list, unit: s
% tk_pre_center = [15.5];
% tk_pre_lst = [15:0.1:16]; % Pre-spike time list, unit: s

% dt_lst = [3000];
dt_lst = [-100, -500, -1500, -2500];
...
for k = 1:length(dt_lst)
    dt = dt_lst(k);
    [t_Ca_lst, V_lst, Ca_sys_lst] = run_simulation_k(tk_pre_lst, tk_pre_center, t_end, dt_lst(k), k, paras);
%     save(sprintf('Result/Result_04_28/BTSP/BTSP_dynamic_dt=%dms.mat', dt), 'dt', ...
%      't_Ca_lst', 'Ca_lst', 't_lst', 'y_lst', 'c_MP_lst', 'c_E_lst');
    % fig = figure(k);
    plot(t_Ca_lst, V_lst, 'DisplayName', sprintf('%d ms', dt), 'LineWidth',2.5,...
    'Color', color_lst(k,:));
...
set(axes1,'FontSize',28,'LineWidth',2.5,'XTick',...
    ...
    [-80 -60 -40 -20 0 20 40],'YTickLabel',{'-80','-60','-40','-20','0', '20', '40'});
% 创建 legend
...
function [t_Ca_lst, V_lst, Ca_sys_lst] = run_simulation_k(tk_pre_lst, tk_pre_center, t_end, dt, k, paras)
...
I_stim_func = @(t) 4000 * any(((tk_post_lst + 300/1e3) > t) .* (tk_post_lst < (t))) + 42;
...
dc_model = @(t, y) V_dynamic_Brunel_stoc(y, 0, 0, I_stim_func(t), 0.412, 2);
