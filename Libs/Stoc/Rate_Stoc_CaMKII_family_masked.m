function [R_mat_all] = Rate_Stoc_CaMKII_family_masked(t, y, c_Ca, paras)
...
%        compute_Prob_special.m
    ...
    p_MP_E_base = paras.c_E0_base * p_MP / (p_MP * c_MPs + paras.K_MP_E);
    ...
    R_mat_all(1, end) = paras.alpha_P_auto * c_EP / (paras.K_m50_EP + c_EP) * c_E * paras.coef_V;
    % Calcium-induced dephosphorylation
    alpha_P_Ca = alpha_P_func_Retuning(c_Ca, paras);
    R_mat_all(2, end) = (alpha_P_Ca * c_EP) * paras.coef_V;
    
    mask = paras.CaMKII_mask;
    % Dynamics of CaMKII
    ...
    R_mat_all(2:(n+1), 1:end-1) = mask .* (y_CaMKII(1, :) .* p_MP_ADP * paras.beta_A .* y_CaMKII(2:(n+1), :)...
                          + y_CaMKII(1, :) .* p_M_ATP * paras.alpha_A .* (1 - y_CaMKII(2:(n+1), :)));
    % Self phosphorylation with CaM
    R_mat_all((n+2):(2*n+1), 1:end-1) = mask .* (y_CaMKII(1, :) .* p_MPA_CaM * paras.beta_AC .* y_CaMKII(2:(n+1), :) ...
                                + y_CaMKII(1, :) .* p_MA_CaM * paras.alpha_AC .* (1 - y_CaMKII(2:(n+1), :)));
    % Neighbouring phosphorylation (catalyst with CaM)
    R_mat_all((2*n+2):(3*n+1), 1:end-1) = mask .* (y_CaMKII(1, :) .* p_MA_CaM * (p_MA_CaM + p_M_CaM) * paras.alpha_AO .* (1 - y_CaMKII(2:(n+1), :)) .* (1 - [y_CaMKII(3:(n+1), :); y_CaMKII(2, :)])... 
    ...
    R_mat_all((3*n+2):(4*n+1), 1:end-1) = mask .* (y_CaMKII(1, :) .* p_MA_CaM * paras.alpha_AP .* (1 - y_CaMKII(2:(n+1), :)) .* [y_CaMKII(3:(n+1), :); y_CaMKII(2, :)]... 
                                  + y_CaMKII(1, :) .* p_MPA_CaM * paras.beta_AP .* y_CaMKII(2:(n+1), :) .* [y_CaMKII(3:(n+1), :); y_CaMKII(2, :)]);
    % Dephosphoryolation with Baseline PP
    R_mat_all((4*n+2):(5*n+1), 1:end-1) = mask .* (y_CaMKII(1, :) .* p_MP_E_base * paras.alpha_E .* y_CaMKII(2:(n+1), :)... 
                                  + y_CaMKII(1, :) .* p_M_EP * (gamma_c_E0 - 1) * paras.beta_E .* (1 - y_CaMKII(2:(n+1), :)));
    % Dephosphoryolation with active PP
    R_mat_all((5*n+2):(6*n+1), 1:end-1) = mask .* (y_CaMKII(1, :) .* p_MP_E * paras.alpha_E .* y_CaMKII(2:(n+1), :)... 
