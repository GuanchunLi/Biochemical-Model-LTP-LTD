function [R_CaV] = Rate_CaV12_channel(n_CaV, V, cp)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
% n_CaV -- number of CaV12 channels in different states: 7*1 array
% R_CaV-- reaction rates of CaV12 channels: 20*1 array
...
k3 = exp(-(V+40)/3) ./ (3 * (1+exp(-(V+40)/3)));
k3_p = k3;

R_V = 10 + 4954 * exp(V/15.6);
% Pr = 1 / (1+exp((V+40)/4));
Pr = 1 - 1 / (1+exp(-(V+40)/4));
Ps = 1 / (1+exp(-(V+40)/11.32));
