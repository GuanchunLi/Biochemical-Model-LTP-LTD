function [t_new, idx] = Reaction_stoc_wo_transpp(t, y, c_Ca, c_E0, c_CaM, paras)
...
%        compute_Prob_special.m
    ...
    [nx, ny] = size(y);
    R_mat = zeros(size(y));
    R_mat(1, :) = paras.mu * (1 - y(1, :)) + paras.nu * ((sum(y(2:end, :), 1) < 0.2) .* y(1, :)) * (p_M+p_M_ATP)^n;
    ...
    R_mat_all(1, :) = R_mat(1, :);
    R_mat_all(2:(n+1), :) = R_mat(2:(n+1), :) + R_mat((n+2):(2*n+1), :) + R_mat((2*n+2):(3*n+1), :);
