function [t_new, idx] = Reaction_stoc_wi_transpp(t, y, c_Ca, c_E0, c_CaM, paras)
...
%        compute_Prob_special.m
    ...
    R_mat((3*n+2):(4*n+1), :) = y(1, :) .* p_MA_CaM * (p_MPA_CaM + p_M_CaM) * paras.alpha_AO .* (1 - y(2:(n+1), :)) .* (1 - [y(3:(n+1), :); y(2, :)])... 
                              + y(1, :) .* p_MPA_CaM * (p_MPA_CaM + p_M_CaM) * paras.beta_AO .* y(2:(n+1), :) .* (1 - [y(3:(n+1), :); y(2, :)]);
